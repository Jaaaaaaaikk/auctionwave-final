...
export default defineEventHandler(async (event) => {
    ...
    try {
        // Retrieve only 'Offer Pending' offers for the specified auction listing, including user info, offer details, profile image, and limited offer images
        const [offersRows] = await pool.query(`
            WITH LatestOffers AS (
                ...
                WHERE 
                    o.listing_id = ? AND o.review_status = 'Offer Pending'
            )
            SELECT 
                lo.user_id,
                lo.full_name,
        ...
        return { offers: offersRows, currentUserId: userId };
    } catch (error) {
